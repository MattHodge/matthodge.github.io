...
There are many third party scripts available for Hubot to extend its functionality. You can search for them on the [Node.js package manager site](https://www.npmjs.com/search?q=hubot)(npm) or on [GitHub](https://github.com/hubot-scripts). We will use the `Install-HubotScript` function to install 3 useful scripts.
